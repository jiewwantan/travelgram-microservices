...
describe('FeedItemComponent', () => {
  ...
  it('should display the destination', () => {
    const app = fixture.nativeElement;
    const paragraphs = app.querySelectorAll('p');
    expect(([].slice.call(paragraphs)).map((x) => x.innerText)).toContain(feedItemMocks[0].destination);
  });

  it('should display the country', () => {
    const app = fixture.nativeElement;
    const paragraphs = app.querySelectorAll('p');
    expect(([].slice.call(paragraphs)).map((x) => x.innerText)).toContain(feedItemMocks[0].country);
  });

  it('should display beenthere', () => {
    const app = fixture.nativeElement;
    const paragraphs = app.querySelectorAll('p');
    expect(([].slice.call(paragraphs)).map((x) => x.innerText)).toContain(feedItemMocks[0].beenthere);
  });
});
