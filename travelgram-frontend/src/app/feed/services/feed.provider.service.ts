...
export class FeedProviderService {
  ...
  async uploadFeedItem(destination: string, country: string, file: File, beenthere: string): Promise<any> {
    const res = await this.api.upload('/feed', file, {destination: destination, country: country, url: file.name, beenthere: beenthere});
